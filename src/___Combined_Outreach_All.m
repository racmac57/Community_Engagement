...
let
    ...
    PythonETLFile = try let
        // Get all CSV files in the output directory
        Source = Folder.Files(OutputFolder),
    FilteredFiles = Table.SelectRows(
        Source, each Text.StartsWith([Name], "community_engagement_data_") and
                    Text.EndsWith([Name], ".csv")),
    SortedFiles = Table.Sort(FilteredFiles,
                             {{"Date modified", Order.Descending}}),
    LatestFile = if Table.RowCount(SortedFiles) >
                     0 then OutputFolder &SortedFiles{0}[Name] else error
                     "No Python ETL output files found" in LatestFile otherwise
                     // Fallback to a specific filename pattern if dynamic
                     // discovery fails
                     OutputFolder &
                 "community_engagement_combined_" &
                 Date.ToText(DateTime.Date(DateTime.LocalNow()), "yyyy-MM-dd") &
                 ".csv",

    // === 2. Read Python ETL CSV Output ===
    CSVSource = Csv.Document(File.Contents(PythonETLFile),
                             [
                               Delimiter = ",", Columns = null,
                               Encoding = 65001, QuoteStyle = QuoteStyle.None
                             ]),
    HeadersPromoted = Table.PromoteHeaders(CSVSource,
                                           [PromoteAllScalars = true]),

    // === 3. Data Type Transformations ===
    // Handle datetime strings from Python ETL (format: "YYYY-MM-DD HH:MM:SS")
    DateParsed = Table.TransformColumns(
        HeadersPromoted,
        {{"date",
          each if _ = null or
                      _ = "" then null else try DateTime
                              .Date(DateTime.FromText(_))
                                  otherwise Date.FromText(Text.Start(_, 10)),
          type date}}),

    // Handle time columns safely (may be in various formats from Python)
    TimeParsed = Table.TransformColumns(
        DateParsed,
    ...
    SafeDuration = Table.TransformColumns(
        TypedData,
        {{"duration_hours",
          each if _ = null or
                      _ = "" or
                          Text.From(_) = "nan" then 0.5 else try Number.From(_)
                                             otherwise 0.5,
          type number}}),

    // Handle any attendee_count conversion issues
    SafeAttendees = Table.TransformColumns(
        SafeDuration,
    ...
    ValidatedDuration = Table.TransformColumns(
        FilteredData,
        {{"Event Duration (Hours)",
          each if _ = null or _ <= 0 then 0.5 else if _ >
                                  24 then 8.0 else Number.Round(_, 2),
          type number}}),

    // Ensure attendee count is at least 1 if null/zero
    ValidatedAttendees = Table.TransformColumns(
        ValidatedDuration,
        {{"Number of Police Department Attendees",
          each if _ = null or _ <= 0 then 1 else _, Int64.Type}}),

    // === 6. Final Sort and Output ===
    // Sort by date ascending (same as original M code)
    FinalData = if Table.RowCount(ValidatedAttendees) >
                0 then Table.Sort(ValidatedAttendees,
                                  {{"Date",
                                    Order.Ascending}}) else ValidatedAttendees
